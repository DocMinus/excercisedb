# just a test.
# would need to changed each function to include the session as an argument
# and potentially also the user_id (and more?)
# i.e. not using this for now.
...
def get_latest_exercises(user_id: int, Session) -> pd.DataFrame:
    session = Session()
...
def save_exercise(exercise_id, notes, date, set_details, Session):
